const Utils = {
  updateRecord(property, newValue, colConfig, record) {
    ...
    const setter = config.setter ? (value, property, config) => {
      if (typeof config.setter === 'string') {
        record[config.setter](value, property, config.record);
      } else {
        config.setter.call(record, value, property, config.record);
      }
    } : () => {
      path.split('.').reduce((previous, current, index, array) => {
